...
import sk.digitalh.grt.toolkit.dto.grt.GRTCaliber;
...
public class GRTCaliberTest {


    private static final Logger LOGGER = LogManager.getLogger(GRTCaliberTest.class.getName());
    ...
    void testWriteAndExistence() {
        ...
        try {
            xmlMapper.writeValue(new File("target/simple_grt_caliber.caliber"),
                    new GRTProjectile());
        } catch (IOException e) {
            LOGGER.error(e);
            throw new RuntimeException(e);
        }
        File file = new File("target/simple_grt_caliber.caliber");
        assertNotNull(file);
    }
    ...
    void testAllResourceFilesForRead() {
        ...
        for (File file : filesInFolder) {
            ...
            // *.caliber files
            GRTCaliber grtCaliber = null;

            try {
                grtCaliber = xmlMapper.readValue(xml, GRTCaliber.class);
                assertNotNull(grtCaliber);
            } catch (JsonProcessingException e) {
                LOGGER.error(e);
                throw new RuntimeException(e);
            }

            LOGGER.debug(grtCaliber.toString());
        }
    }
